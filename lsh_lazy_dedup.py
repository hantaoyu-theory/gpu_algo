"""
GPU-Accelerated LSH  (E2LSH for L2, SimHash for cosine)  — LAZY-DEDUP VARIANT
=========================================================
Pipeline
--------
Build
...
Query
  ...
  [stage 8 removed: no upfront dedup — process all C = L*max_cands candidates including duplicates]
  9. gather_vectors        – index into dataset with candidate ids
  10. rerank_distances     – ||q - c||^2 via GEMM identity (q_sq + c_sq - 2*q·c)
  11. topk_selection       – argpartition + argsort + cheap CPU dedup of final k results
...
class LSHIndex:
    ...
    def search(self, Q: cp.ndarray, k: int) -> Tuple[cp.ndarray, cp.ndarray]:
        ...
        # stage 8 removed: skip upfront dedup.
        # Replace -1 (invalid/empty slots) with 0 to avoid OOB in gather.
        # Duplicates are left in — they will be reranked multiple times but
        # the final top-k dedup in stage 11 corrects the output.
        n_valid = int((all_cands >= 0).sum(axis=1).max())
        n_valid = max(n_valid, k)
        padded_gpu = cp.where(all_cands >= 0, all_cands, cp.int32(0))  # (n, C)
        print(f"  [lazy_dedup] C={C}  n_valid(max)={n_valid}  "
              f"duplicate_fraction={1 - n_valid/C:.2%}")

        # ── stage 9 ──────────────────────────────────────────────────────────
        # Gather candidate vectors for all C slots (including duplicates).
        # padded_gpu: (n, C) indices  →  cand_vecs: (n, C, d)
        prof.start("9_query/gather_vectors")
        flat_idx  = padded_gpu.reshape(-1)                          # (n * C,)
        cand_vecs = self._dataset[flat_idx]                         # (n*C, d)
        cand_vecs = cand_vecs.reshape(n, C, self._d)                # (n, C, d)
        ...
        if self.p.metric in ("l2", "sqeuclidean"):
            ...
            dists = cp.maximum(q_sq + c_sq - 2.0 * dot, 0.0)       # (n, C)
        else:                                                       # cosine distance
            eps   = 1e-8
            Q_n   = Q        / (cp.linalg.norm(Q,        axis=1, keepdims=True) + eps)
            C_n   = cand_vecs / (cp.linalg.norm(cand_vecs, axis=2, keepdims=True) + eps)
            dists = 1.0 - cp.einsum("nd,ncd->nc", Q_n, C_n)        # (n, C)

        # Zero out distances for padded (invalid) slots so they sort to the end.
        # We use a large sentinel distance rather than masking after topk.
        invalid_mask = (all_cands < 0)                              # (n, C) bool
        dists = cp.where(invalid_mask, cp.float32(1e18), dists)
        prof.stop("10_query/rerank_distances")

        # ── stage 11 ─────────────────────────────────────────────────────────
        # Select top-(k * oversample) by distance, then CPU-dedup to k unique.
        # We oversample by 4x to ensure we have enough unique indices after
        # removing duplicates from the top candidates.
        prof.start("11_query/topk_selection")
        oversample = min(k * 4, C)
        top_local  = cp.argpartition(dists, oversample - 1, axis=1)[:, :oversample]
        ...
        top_global = cp.take_along_axis(padded_gpu, top_local, axis=1)  # (n, oversample)
        prof.stop("11_query/topk_selection")

        # ── stage 11b: cheap CPU dedup of top-(k*4) results ──────────────────
        # k=10 and oversample=40, so this is negligible (~0ms).
        top_np  = cp.asnumpy(top_global)                            # (n, oversample)
